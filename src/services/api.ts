...
export const api = { auth: {
  ...
  google: (b: { credential: string }) => request<{ accessToken?: string; user?: Record<string, unknown>; isExistingUser?: boolean; googleData?: { googleId: string; email: string; firstName: string; lastName: string } }>("/auth/google", { method: "POST", body: JSON.stringify(b) }),
  googleRegister: (b: { googleId: string; email: string; firstName: string; lastName: string; companyName: string; password: string }) => request<{ accessToken: string; user: Record<string, unknown> }>("/auth/google/register", { method: "POST", body: JSON.stringify(b) }),
